...
void select_by_distance(point_list pts[], int j, int s, int pivot, float *pr)  // check the last parameter here and      UPDATE: (Seems to be correct as of now, but give a thorough check)
                                                                             //  the way it's going to be accessed.
{
    ...
    {   
        std :: cout << "squared difference here is :";
        dist[i++] = sqrt( ( pow(pts[index].x - pts[j].x,2) ) + ( pow(pts[index].y - pts[j].y,2) ) ) ;
    ...
    std::cout << "\n" << pivot << "th smallest element is : " << dist[pivot-1] << std::endl ;
    *pr = dist[pivot-1];
    ...
    for (i = 0; i < n - 1;) // check for n. (TO DO)                      // i and ii tracks the current point in the vantage point tree.
    {
        ...
        while (j < n) 
        {
            ...
            if (s > 1) 
            {
                ...
                random_point(pts, j, s);                /*
                                                                            In the first pass, pts array (untampered, j=0, s= 15)
                                                                            Returns with the first element as the pivot element., then select the pp-1 element to be smallest distance from the point
                                                                            ( pts[j].x,pts[j].y )
                                                        */
                ...
                select_by_distance(pts, j, s, pp-1, &pr);                               
                                                        /*
                ...
                std::cout << "Pivot radius" << pr << std::endl;
